...
docker build --build-arg COMMIT_HASH=$TRAVIS_COMMIT --build-arg BRANCH=$DEPLOY_BRANCH --build-arg REPOSITORY=$REPOSITORY --no-cache -t fossasia/susi_gassistantbot:$TRAVIS_COMMIT .
docker login -u="$DOCKER_USERNAME" -p="$DOCKER_PASSWORD"
docker tag fossasia/susi_gassistantbot:$TRAVIS_COMMIT fossasia/susi_gassistantbot:latest-$DEPLOY_BRANCH
echo ">>> Pushing docker image"
docker push fossasia/susi_gassistantbot
echo ">>> Updating deployment"
kubectl set image deployment/google --namespace=google google=fossasia/susi_gassistantbot:$TRAVIS_COMMIT
